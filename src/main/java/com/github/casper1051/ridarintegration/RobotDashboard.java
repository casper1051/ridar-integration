package com.github.casper1051.ridarintegration;import com.intellij.openapi.project.Project;
...
import h.D.D;import javax.swing.plaf.basic.BasicScrollBarUI;import javax.swing.*;
...
import java.util.Map;public class RobotDashboard extends JPanel {    private static String ROBOT_IP = loadTargetIp();
    private static int ROBOT_PORT = 8080;    
    ...
    private static final String CARD_OTHER = "OTHER";    private final JLabel statusLabel = new JLabel("Checking Connection...", SwingConstants.CENTER);
    private final JLabel batteryLabel = new JLabel("BATTERY: NR%", SwingConstants.CENTER);    private final JTextArea logArea = new JTextArea();    private final CardLayout mainCardLayout = new CardLayout();
    ...
    private String currentCard = CARD_ROOT;    private final JLabel[] motorPosLabels = new JLabel[4];
    private final JSlider[] motorSliders = new JSlider[4];    private final JSlider[] servoSliders = new JSlider[4];
    private final JToggleButton[] servoEnableToggles = new JToggleButton[4];
    private boolean servoPositionsLoaded = false;    private final JLabel[] digitalStateLabels = new JLabel[10];    private static final int MAX_SAMPLES = 200;
    ...
    private AnalogGraphPanel analogGraphPanel;    private final List<Double> gyroXData = Collections.synchronizedList(new ArrayList<>());
    ...
    private final List<Double> accelZData = Collections.synchronizedList(new ArrayList<>());    private final JLabel[] gyroLabels = new JLabel[3];
    private final JLabel[] accelLabels = new JLabel[3];    private JCheckBox chkX, chkY, chkZ;
    ...
    private ImuGraphPanel accelGraphPanel;    private volatile boolean isReachable = false;
    ...
    private float uiScale = 1.0f;    public RobotDashboard() {
        ...
        ));        JLabel titleLabel = new JLabel("RIDAR PANEL");
        titleLabel.setForeground(Color.WHITE);
        titleLabel.setFont(new Font("SansSerif", Font.BOLD, 13));        statusLabel.setOpaque(true);
        ...
        batteryLabel.setFont(new Font("Monospaced", Font.BOLD, 11));
        batteryLabel.setBorder(new CompoundBorder(
                new LineBorder(BORDER_COLOR, 1),
                new EmptyBorder(4, 10, 4, 10)
        ));        updateStatusBadge(false, false, "INITIALIZING");        JButton btnRedo = createScaleButton("\u21bb"); // ↻ symbol
        btnRedo.setToolTipText("Redo last action");
        btnRedo.addActionListener(e -> sendSocketCommand("control", "system", "redo", 0, 0));        JToggleButton btnPlayPause = new JToggleButton("\u23ef"); // ⏯ symbol
        btnPlayPause.setToolTipText("Play / Pause execution");
        btnPlayPause.setBackground(BUTTON_BG);
        btnPlayPause.setForeground(TEXT_COLOR);
        btnPlayPause.setFocusPainted(false);
        btnPlayPause.setBorder(new CompoundBorder(
                new LineBorder(BORDER_COLOR, 1),
                new EmptyBorder(3, 8, 3, 8)
        ));
        btnPlayPause.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
        btnPlayPause.addActionListener(e -> {
            boolean isPlaying = btnPlayPause.isSelected();
            if (isPlaying) {
                btnPlayPause.setBackground(OK_GREEN);
                sendSocketCommand("control", "system", "play", 0, 0);
            } else {
                btnPlayPause.setBackground(BUTTON_BG);
                sendSocketCommand("control", "system", "pause", 0, 0);
            }
        });        JTextField targetIpField = new JTextField(15);
        targetIpField.setText(ROBOT_IP + ":" + ROBOT_PORT);
        targetIpField.setBackground(BUTTON_BG);
        targetIpField.setForeground(TEXT_COLOR);
        targetIpField.setCaretColor(TEXT_COLOR);
        targetIpField.setFont(new Font("Monospaced", Font.PLAIN, 11));
        targetIpField.setToolTipText("Enter target IP:Port (e.g., 192.168.125.1:8080)");
        targetIpField.setBorder(new CompoundBorder(
                new LineBorder(BORDER_COLOR, 1),
                new EmptyBorder(3, 6, 3, 6)
        ));
        targetIpField.setMinimumSize(new Dimension(150, targetIpField.getHeight()));
        targetIpField.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
            private void parseAndUpdate() {
                String text = targetIpField.getText().trim();
                if (text.contains(":")) {
                    String[] parts = text.split(":", 2);
                    String ipCandidate = parts[0].trim();
                    try {
                        int portCandidate = Integer.parseInt(parts[1].trim());
                        if (!ipCandidate.isEmpty() && portCandidate > 0 && portCandidate <= 65535) {
                            ROBOT_IP = ipCandidate;
                            ROBOT_PORT = portCandidate;
                        }
                    } catch (NumberFormatException ignored) {
                        // Invalid port number while typing; ignore until valid
                    }
                }
            }
            @Override public void insertUpdate(javax.swing.event.DocumentEvent e) { parseAndUpdate(); }
            @Override public void removeUpdate(javax.swing.event.DocumentEvent e) { parseAndUpdate(); }
            @Override public void changedUpdate(javax.swing.event.DocumentEvent e) { parseAndUpdate(); }
        });        JButton btnStepForward = createScaleButton("\u23ed"); // ⏭ symbol
        btnStepForward.setToolTipText("Step one frame/command forward");
        btnStepForward.addActionListener(e -> sendSocketCommand("control", "system", "step_forward", 0, 0));        JPanel controlGroup = new JPanel(new FlowLayout(FlowLayout.CENTER, 2, 0));
        controlGroup.setBackground(PANEL_BG);
        controlGroup.add(targetIpField);
        controlGroup.add(btnRedo);
        controlGroup.add(btnPlayPause);
        controlGroup.add(btnStepForward);
        JButton btnScaleDown = createScaleButton("\u2212");
        btnScaleDown.setToolTipText("Zoom out (Ctrl/Cmd -)");
        btnScaleDown.addActionListener(e -> adjustScale(-SCALE_STEP));        JButton btnScaleReset = createScaleButton("=");
        btnScaleReset.setToolTipText("Reset zoom");
        btnScaleReset.addActionListener(e -> resetScale());        JButton btnScaleUp = createScaleButton("+");
        btnScaleUp.setToolTipText("Zoom in (Ctrl/Cmd +)");
        btnScaleUp.addActionListener(e -> adjustScale(SCALE_STEP));        JPanel scaleGroup = new JPanel(new FlowLayout(FlowLayout.CENTER, 2, 0));
        scaleGroup.setBackground(PANEL_BG);
        scaleGroup.add(btnScaleDown);
        scaleGroup.add(btnScaleReset);
        scaleGroup.add(btnScaleUp);        JButton btnResetUi = createStyledButton("Reset UI");
        ...
        btnResetUi.addActionListener(e -> resetUiState());        JPanel headerRightPanel = new JPanel(new FlowLayout(FlowLayout.RIGHT, 6, 0));
        headerRightPanel.setBackground(PANEL_BG);        headerRightPanel.add(batteryLabel);        headerRightPanel.add(statusLabel);
        headerRightPanel.add(controlGroup); // Added to the left of zoom buttons
        headerRightPanel.add(scaleGroup);
        headerRightPanel.add(btnResetUi);        topHeader.add(titleLabel, BorderLayout.WEST);
        ...
        navBackButton.setVisible(false);        navTitleLabel.setForeground(Color.WHITE);
        navTitleLabel.setFont(new Font("SansSerif", Font.BOLD, 13));
        navTitleLabel.setHorizontalAlignment(SwingConstants.LEFT);
        navTitleLabel.setBorder(new EmptyBorder(0, 10, 0, 0));        navHeader.add(navBackButton, BorderLayout.WEST);
        navHeader.add(navTitleLabel, BorderLayout.CENTER);        JPanel topContainer = new JPanel(new BorderLayout());
        ...
        initEffectorsViews();
        initSensorsViews();
        initOtherView();        JPanel contentWrapper = new JPanel(new BorderLayout());
        ...
        logArea.setFont(new Font("Monospaced", Font.PLAIN, 11));        JScrollPane logScroll = createStyledScrollPane(logArea);
        ...
        mainHorizontalSplit.setContinuousLayout(true);        add(topContainer, BorderLayout.NORTH);
        add(mainHorizontalSplit, BorderLayout.CENTER);        showCard(CARD_ROOT, "Overview");
        ...
        homePanel.setBorder(new EmptyBorder(12, 12, 12, 12));        JPanel cardNav = createCardPanel("Quick Navigation");
        cardNav.setLayout(new GridLayout(3, 1, 8, 8));        JButton quickEffectors = createStyledButton("Open Effector Controls (Motors & Servos)");
        JButton quickSensors = createStyledButton("Open Sensor Monitor (Digital, Analog, IMU)");
        JButton quickTools = createStyledButton("Open Diagnostic Tools");        quickEffectors.addActionListener(e -> navigateTo(CARD_EFFECTORS_MENU, "Effectors"));
        quickSensors.addActionListener(e -> navigateTo(CARD_SENSORS_MENU, "Sensors"));
        quickTools.addActionListener(e -> navigateTo(CARD_OTHER, "Tools"));        cardNav.add(quickEffectors);
        cardNav.add(quickSensors);
        cardNav.add(quickTools);        JPanel cardInfo = createCardPanel("General Info");
        ...
        cardInfo.add(new JLabel("RIDAR Bridge C++ Framework", SwingConstants.LEFT));
        cardInfo.add(new JLabel("Firmware-Compliant", SwingConstants.LEFT));        for (Component c : cardInfo.getComponents()) {
            c.setForeground(TEXT_COLOR);
            c.setFont(new Font("SansSerif", Font.PLAIN, 12));
        }        homePanel.add(cardNav);
        homePanel.add(Box.createRigidArea(new Dimension(0, 10)));
        homePanel.add(cardInfo);        mainContentPanel.add(createStyledScrollPane(homePanel), CARD_ROOT);
        ...
        return btn;
    }    private static String loadTargetIp() {
        File projectDir = getCLionProjectDirectoryStatic();
        File configFile = new File(projectDir, ".cts/config.json");        if (configFile.exists()) {
            try (BufferedReader reader = new BufferedReader(new FileReader(configFile, StandardCharsets.UTF_8))) {
                ...
                String content = sb.toString();                // Simple robust extraction for "TargetIP"
                int keyIdx = content.indexOf("\"TargetIP\"");
                if (keyIdx != -1) {
                    int colonIdx = content.indexOf(":", keyIdx);
                    int firstQuote = content.indexOf("\"", colonIdx);
                    int secondQuote = content.indexOf("\"", firstQuote + 1);
                    if (firstQuote != -1 && secondQuote != -1) {
                        String ip = content.substring(firstQuote + 1, secondQuote).trim();
                        if (!ip.isEmpty()) {
                            return ip;
                        }
                    }
        ...
        return "192.168.125.1"; // Fallback IP
    }    private static File getCLionProjectDirectoryStatic() {
        try {
        ...
        menuPanel.setBorder(new EmptyBorder(12, 12, 12, 12));        JButton btnMotorsItem = createMenuItemButton("Motors (Ports 0-3)");
        JButton btnServosItem = createMenuItemButton("Servos (Ports 0-3)");        btnMotorsItem.addActionListener(e -> navigateTo(CARD_EFFECTORS_MOTORS, "Motors"));
        ...
        menuPanel.add(btnServosItem);        mainContentPanel.add(menuPanel, CARD_EFFECTORS_MENU);        
        ...
        motorsListContainer.setBorder(new EmptyBorder(8, 8, 8, 8));        for (int i = 0; i <= 3; i++) {
            final int port = i;
            JPanel mCard = createCardPanel("Motor Port " + port);
            mCard.setLayout(new BoxLayout(mCard, BoxLayout.Y_AXIS));            JSlider speedSlider = new JSlider(-1500, 1500, 0);
            speedSlider.setBackground(PANEL_BG);
            speedSlider.setForeground(TEXT_COLOR);
            speedSlider.setMajorTickSpacing(750);
            speedSlider.setPaintTicks(true);
            speedSlider.setPaintLabels(true);
            motorSliders[port] = speedSlider;            JLabel speedValLbl = new JLabel("Speed: 0", SwingConstants.CENTER);
            speedValLbl.setForeground(ACCENT_BLUE);
            speedValLbl.setFont(new Font("Monospaced", Font.BOLD, 12));
            speedValLbl.setAlignmentX(Component.CENTER_ALIGNMENT);            speedSlider.addChangeListener(e -> {
            ...
            ctrlRow.setBackground(PANEL_BG);            JButton btnStop = createStyledButton("STOP");
            ...
            motorPosLabels[port] = posLabel;            JButton btnClearPos = createStyledButton("Clear Pos");
            ...
            ctrlRow.add(btnClearPos);            mCard.add(speedSlider);
            mCard.add(speedValLbl);
            mCard.add(ctrlRow);            motorsListContainer.add(mCard);
        ...
        servosListContainer.setBorder(new EmptyBorder(8, 8, 8, 8));        for (int i = 0; i <= 3; i++) {
            final int port = i;
            JPanel sCard = createCardPanel("Servo Port " + port);
            sCard.setLayout(new BoxLayout(sCard, BoxLayout.Y_AXIS));            JSlider angleSlider = new JSlider(0, 180, 90);
            angleSlider.setBackground(PANEL_BG);
            angleSlider.setForeground(TEXT_COLOR);
            angleSlider.setMajorTickSpacing(45);
            angleSlider.setPaintTicks(true);
            angleSlider.setPaintLabels(true);
            servoSliders[port] = angleSlider;            JLabel angleLbl = new JLabel("Angle: 90°", SwingConstants.CENTER);
            angleLbl.setForeground(ACCENT_BLUE);
            angleLbl.setFont(new Font("Monospaced", Font.BOLD, 12));
            angleLbl.setAlignmentX(Component.CENTER_ALIGNMENT);            JPanel ctrlRow = new JPanel(new FlowLayout(FlowLayout.CENTER, 6, 4));
            ctrlRow.setBackground(PANEL_BG);            JToggleButton toggleEnable = new JToggleButton("Enabled");
            ...
            servoEnableToggles[port] = toggleEnable;            toggleEnable.addActionListener(e -> {
            ...
            ctrlRow.add(btnCutPower);            sCard.add(angleSlider);
            sCard.add(angleLbl);
            sCard.add(ctrlRow);            servosListContainer.add(sCard);
        ...
        menuPanel.setLayout(new BoxLayout(menuPanel, BoxLayout.Y_AXIS));
        menuPanel.setBackground(BG_DARK);
        menuPanel.setBorder(new EmptyBorder(12, 12, 12, 12));        JButton btnDigitalItem = createMenuItemButton("Digital I/O");
        JButton btnAnalogItem = createMenuItemButton("Analog Sensors");
        JButton btnImuItem = createMenuItemButton("IMU Waveforms");        btnDigitalItem.addActionListener(e -> navigateTo(CARD_SENSORS_DIGITAL, "Digital"));
        btnAnalogItem.addActionListener(e -> navigateTo(CARD_SENSORS_ANALOG, "Analog"));
        btnImuItem.addActionListener(e -> navigateTo(CARD_SENSORS_IMU, "IMU Waveforms"));        menuPanel.add(btnDigitalItem);
        menuPanel.add(Box.createRigidArea(new Dimension(0, 8)));
        menuPanel.add(btnAnalogItem);
        menuPanel.add(Box.createRigidArea(new Dimension(0, 8)));
        menuPanel.add(btnImuItem);        mainContentPanel.add(menuPanel, CARD_SENSORS_MENU);        
        JPanel digitalPanel = new JPanel(new GridLayout(0, 1, 6, 6));
        digitalPanel.setBackground(BG_DARK);
        digitalPanel.setBorder(new EmptyBorder(8, 8, 8, 8));        for (int i = 0; i <= 9; i++) {
            JPanel p = new JPanel(new BorderLayout(8, 0));
            p.setBackground(PANEL_BG);
            p.setBorder(new CompoundBorder(new LineBorder(BORDER_COLOR), new EmptyBorder(6, 10, 6, 10)));            JLabel name = new JLabel("Digital Port " + i);
            name.setForeground(TEXT_COLOR);            JLabel state = new JLabel("0 (OFF)", SwingConstants.CENTER);
            state.setOpaque(true);
            state.setBackground(DANGER_RED);
            state.setForeground(Color.WHITE);
            state.setFont(new Font("Monospaced", Font.BOLD, 11));
            state.setBorder(new EmptyBorder(2, 8, 2, 8));            digitalStateLabels[i] = state;            p.add(name, BorderLayout.WEST);
        ...
        analogPanel.setBackground(BG_DARK);        JPanel selectorBar = new JPanel(new FlowLayout(FlowLayout.LEFT, 8, 4));
        selectorBar.setBackground(PANEL_BG);        String[] sensorOptions = {"Analog 0", "Analog 1", "Analog 2", "Analog 3", "None"};
        sensor1Select = new JComboBox<>(sensorOptions);
        sensor2Select = new JComboBox<>(sensorOptions);
        sensor1Select.setSelectedIndex(0);
        sensor2Select.setSelectedIndex(1);        styleComboBox(sensor1Select);
        styleComboBox(sensor2Select);        JLabel l1 = new JLabel("S1 (Blue):"); l1.setForeground(TEXT_COLOR);
        JLabel l2 = new JLabel("S2 (Red):"); l2.setForeground(TEXT_COLOR);        selectorBar.add(l1);
        selectorBar.add(sensor1Select);
        selectorBar.add(l2);
        selectorBar.add(sensor2Select);        analogGraphPanel = new AnalogGraphPanel();
        analogPanel.add(selectorBar, BorderLayout.NORTH);
        analogPanel.add(analogGraphPanel, BorderLayout.CENTER);        mainContentPanel.add(analogPanel, CARD_SENSORS_ANALOG);        
        JPanel imuMasterPanel = new JPanel(new BorderLayout());
        imuMasterPanel.setBackground(BG_DARK);        JPanel imuControls = new JPanel(new FlowLayout(FlowLayout.LEFT, 8, 4));
        imuControls.setBackground(PANEL_BG);
        imuControls.setBorder(new LineBorder(BORDER_COLOR, 1));        imuDisplaySelect = new JComboBox<>(new String[]{"Gyroscope (deg/s)", "Accelerometer (m/s²)"});
        styleComboBox(imuDisplaySelect);        chkX = new JCheckBox("X", true);
        chkY = new JCheckBox("Y", true);
        chkZ = new JCheckBox("Z", true);        styleCheckBox(chkX, ACCENT_BLUE);
        styleCheckBox(chkY, ACCENT_RED);
        styleCheckBox(chkZ, ACCENT_GREEN);        imuDisplaySelect.addActionListener(e -> {
        ...
        chkZ.addActionListener(e -> repaintImuGraphs());        imuControls.add(imuDisplaySelect);
        imuControls.add(chkX);
        imuControls.add(chkY);
        imuControls.add(chkZ);        JPanel imuBadges = new JPanel(new GridLayout(2, 1, 6, 6));
        imuBadges.setBackground(BG_DARK);
        imuBadges.setBorder(new EmptyBorder(4, 4, 4, 4));        JPanel gyroBox = createCardPanel("Gyroscope Live Readout");
        gyroBox.setLayout(new FlowLayout(FlowLayout.LEFT, 12, 2));        JPanel accelBox = createCardPanel("Accelerometer Live Readout");
        accelBox.setLayout(new FlowLayout(FlowLayout.LEFT, 12, 2));        String[] axes = {"X", "Y", "Z"};
        for (int i = 0; i < 3; i++) {
            ...
            gyroBox.add(gyroLabels[i]);            accelLabels[i] = new JLabel(axes[i] + ": 0.00 m/s² ");
        ...
        imuBadges.add(accelBox);        gyroGraphPanel = new ImuGraphPanel(gyroXData, gyroYData, gyroZData, "deg/s", true);
        accelGraphPanel = new ImuGraphPanel(accelXData, accelYData, accelZData, "m/s²", false);        imuGraphCardPanel.add(gyroGraphPanel, "GYRO");
        imuGraphCardPanel.add(accelGraphPanel, "ACCEL");        imuMasterPanel.add(imuControls, BorderLayout.NORTH);
        imuMasterPanel.add(imuGraphCardPanel, BorderLayout.CENTER);
        imuMasterPanel.add(imuBadges, BorderLayout.SOUTH);        mainContentPanel.add(imuMasterPanel, CARD_SENSORS_IMU);
        ...
        otherPanel.setBackground(BG_DARK);        JLabel infoLabel = new JLabel("Use the right terminal pane to run interactive shell commands or SSH into user@" + ROBOT_IP);
        infoLabel.setForeground(TEXT_COLOR);
        infoLabel.setFont(new Font("SansSerif", Font.PLAIN, 12));        otherPanel.add(infoLabel);
        ...
        ActionMap am = getActionMap();        im.put(KeyStroke.getKeyStroke(KeyEvent.VK_EQUALS, menuMask), "scaleIncrease");
        ...
        im.put(KeyStroke.getKeyStroke(KeyEvent.VK_0, menuMask), "scaleReset");        am.put("scaleIncrease", new AbstractAction() {
            @Override
        ...
        if (comp instanceof JComponent) {
            JComponent jc = (JComponent) comp;            Font f = jc.getFont();
            if (f != null) {
        ...
        appendLog("[SYSTEM] Resetting UI state and reconnecting client loops...");        sensor1Data.clear();
        sensor2Data.clear();
        gyroXData.clear(); gyroYData.clear(); gyroZData.clear();
        accelXData.clear(); accelYData.clear(); accelZData.clear();        for (int i = 0; i < 4; i++) {
        ...
        startBackgroundServices();        repaintImuGraphs();
        if (analogGraphPanel != null) analogGraphPanel.repaint();        goToRoot();        appendLog("[SYSTEM] Reset complete.");
        ...
        if (reachable && serverOnline) {
            ...
            statusLabel.setForeground(Color.WHITE);
            statusLabel.setText(" DEVICE REACHABLE | DEBUGGER DISCONNECTED ");
        } else {
            statusLabel.setBackground(DANGER_RED);
            statusLabel.setForeground(Color.WHITE);
            statusLabel.setText(" OFFLINE | " + statusMsg + " ");
        }
    }    private synchronized void updateBatteryBadge(int percent) {
        if (percent >= 90) {
            batteryLabel.setBackground(OK_GREEN);
            batteryLabel.setForeground(Color.WHITE);
            batteryLabel.setText("BATTERY: " + percent + "%");
        } else if (percent > 50) {
            batteryLabel.setBackground(WARN_ORANGE);
            batteryLabel.setForeground(Color.WHITE);
            batteryLabel.setText("BATTERY: " + percent + "%");
        } else {
            batteryLabel.setBackground(DANGER_RED);
            batteryLabel.setForeground(Color.WHITE);
            batteryLabel.setText("BATTERY: " + percent + "%");
        }
        ...
        String json = String.format("{\"category\":\"%s\",\"device\":\"%s\",\"action\":\"%s\",\"port\":%d,\"value\":%d}\n",
                category, device, action, port, value);        appendLog("[CMD OUT] " + json.trim());        new Thread(() -> {
        ...
        try {
            ...
            command.add("1");
            command.add(ip);            ProcessBuilder processBuilder = new ProcessBuilder(command);
        ...
        telemetryThread = new Thread(() -> {
            while (threadsRunning) {
                try {
                    boolean reachable = executePing(ROBOT_IP);                    if (!reachable) {
                        ...
                        try (BufferedReader reader = new BufferedReader(new InputStreamReader(socket.getInputStream(), StandardCharsets.UTF_8))) {
                            ...
                            appendLog("[SYSTEM] Connected to server debugger stream at " + ROBOT_IP + ":" + ROBOT_PORT);                            String line;
        ...
        SwingUtilities.invokeLater(() -> {
            try {
                if (json.contains("\"imu\"")) {
                    ...
                    double gz = extractJsonNumber(json, "gz");                    double ax = extractJsonNumber(json, "ax");
                    double ay = extractJsonNumber(json, "ay");
                    double az = extractJsonNumber(json, "az");                    gyroLabels[0].setText(String.format("X: %.2f deg/s", gx));
                    gyroLabels[1].setText(String.format("Y: %.2f deg/s", gy));
                    gyroLabels[2].setText(String.format("Z: %.2f deg/s", gz));                    accelLabels[0].setText(String.format("X: %.2f m/s²", ax));
                    accelLabels[1].setText(String.format("Y: %.2f m/s²", ay));
                    accelLabels[2].setText(String.format("Z: %.2f m/s²", az));                    synchronized (gyroXData) {
                ...
                }                int batIdx = json.indexOf("\"battery\"");
                if (batIdx != -1) {
                    int colonIdx = json.indexOf(":", batIdx);
                    if (colonIdx != -1) {
                        int commaIdx = json.indexOf(",", colonIdx);
                        int braceIdx = json.indexOf("}", colonIdx);                        int endIdx = -1;
                        if (commaIdx != -1 && braceIdx != -1) {
                            endIdx = Math.min(commaIdx, braceIdx);
                        } else {
                            endIdx = (commaIdx != -1) ? commaIdx : braceIdx;
                        }                        if (endIdx != -1) {
                            String batStr = json.substring(colonIdx + 1, endIdx).trim();
                            try {
                                int batteryValue = Integer.parseInt(batStr);                                updateBatteryBadge(batteryValue);                            } catch (NumberFormatException e) {
                                updateBatteryBadge(-1);
                            }
                        }
                    }
                ...
                if (anaIdx != -1) {
                    ...
                    if (endIdx != -1) {
                        ...
                        int s2Idx = sensor2Select.getSelectedIndex();                        synchronized (sensor1Data) {
    ...
    private class EmbeddedTerminalPanel extends JPanel {
        ...
        private Process shellProcess;        public EmbeddedTerminalPanel() {
            setLayout(new BorderLayout());
            ...
            toolbar.setBackground(PANEL_BG);            JButton btnSsh = createStyledButton("SSH...");
            btnSsh.addActionListener(e -> openSshSession());            JButton btnRestart = createStyledButton("Restart Shell");
            btnRestart.addActionListener(e -> restartShell());            toolbar.add(btnSsh);
            toolbar.add(btnRestart);            add(toolbar, BorderLayout.NORTH);            termWidget = buildTerminalWidget();
            add(termWidget, BorderLayout.CENTER);            startShell(termWidget, 100, 30);            
            ...
            new Thread(() -> {
                try {
                    ...
                    String initCommand = String.format(
                            "stty rows %d columns %d >/dev/null 2>&1; exec %s -l", rows, cols, shell);                    ProcessBuilder pb = isMac()
                            ? new ProcessBuilder("script", "-q", "/dev/null", "/bin/sh", "-c", initCommand)
                            : new ProcessBuilder("script", "-qc", initCommand, "/dev/null");                    Map<String, String> env = pb.environment();
                    env.put("TERM", "xterm-256color");
                    env.put("COLORTERM", "truecolor");
                    //working dir
                    pb.directory(getCLionProjectDirectory());
                    pb.redirectErrorStream(true);                    shellProcess = pb.start();
                    TtyConnector connector = new ScriptPtyConnector(shellProcess);                    SwingUtilities.invokeLater(() -> {
    ...
    private static class ScriptPtyConnector implements TtyConnector {
        ...
        private final Writer writer;        ScriptPtyConnector(Process process) {
        ...
        protected void paintComponent(Graphics g) {
            ...
            int h = getHeight();            g2.setColor(new Color(40, 40, 40));
            for (int i = 0; i < w; i += 40) g2.drawLine(i, 0, i, h);
            for (int i = 0; i < h; i += 40) g2.drawLine(0, i, w, i);            double minVal = Double.MAX_VALUE;
            double maxVal = -Double.MAX_VALUE;            synchronized (sensor1Data) {
            ...
            g2.setColor(color);
            g2.setStroke(new BasicStroke(2.0f));            double range = maxVal - minVal;
            int pts = data.size();            for (int i = 0; i < pts - 1; i++) {
        ...
        private final boolean isGyro;        public ImuGraphPanel(List<Double> x, List<Double> y, List<Double> z, String unit, boolean isGyro) {
            this.dataX = x;
            this.dataY = y;
            this.dataZ = z;
            this.unit = unit;
        ...
        protected void paintComponent(Graphics g) {
            ...
            for (int i = 0; i < w; i += 40) g2.drawLine(i, 0, i, h);
            for (int i = 0; i < h; i += 40) g2.drawLine(0, i, w, i);            double minVal = Double.MAX_VALUE;
            double maxVal = -Double.MAX_VALUE;            synchronized (dataX) {
